...
import { advanceQueue, emptyState, ensureQueue, loadState, recordResult, saveState } from "@/lib/state";
import { streak, summarize, todayFlags } from "@/lib/schedule";
...
export function TodayPage({ session, date }: { session: ClientSession; date: string }) {
  ...
  const onRefresh = useCallback(() => {
    setState((s) => advanceQueue(s, date));
  }, [date]);

  const queue = state.queues[date] ?? { ids: [] };
  const flags = todayFlags(state.states, date, queue);
