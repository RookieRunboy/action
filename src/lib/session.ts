...
export function decodeSession(raw: string | undefined): Session | null {
  ...
  try {
    ...
    if (!s?.id || !s.identity || s.kind !== "oauth" || typeof s.expiresAt !== "number") return null;
...
export async function getSession(): Promise<Session | null> {
  ...
  return decodeSession(jar.get(SID_COOKIE)?.value);
}

/** Route Handler 中读取当前会话 */
export function getSessionFromRequest(req: NextRequest): Session | null {
  return decodeSession(req.cookies.get(SID_COOKIE)?.value);
}
